...
import { Ticket, Gift, Copy, Check, ExternalLink, Sparkles, Package, Users, Boxes, Loader2, ArrowRight } from 'lucide-react'
...
const HOW_IT_WORKS = [
  { icon: Boxes, title: 'We set up a campaign', body: 'Pick which of your warehouse products people can claim — with per-person limits and, if you want, a pre-order option for items not in stock yet.' },
  { icon: Users, title: 'You share one link', body: 'Send the link to your team, clients, or event guests. Optionally lock it to your email domain or a shared password. No accounts, no payments.' },
  { icon: Package, title: 'We fulfil from stock', body: 'Claims flow straight into fulfilment. Anything pre-ordered beyond stock becomes the “to produce” number — no negative balances.' },
]
...
export default function RedeemSection({ company, contact }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const { data } = await supabase
        ...
        .select('id, name, slug, status, picks_per_person, opens_at, closes_at, created_at')
      ...
      if (!cancelled) { setCampaigns(list); setLoading(false) }
    })()
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-5 sm:p-6">
      ...
      <div className="mt-5">
        {interest === 'sent' ? (
          ...
          <p className="text-sm text-gray-500">Ask your account manager to set up your first redeem campaign.</p>
        )}
  ...
  // Partner with live campaigns → manage/share them.
  if (partner && campaigns.length > 0) {
    return (
      <div className="space-y-3">
        ...
        <p className="text-xs text-gray-500">Share a link and people claim merch from your stock. Ask your account manager to add or edit a campaign.</p>
      </div>
    )
  }

  // Partner without campaigns → explainer, no upsell (they already qualify).
  // Non-partner → explainer + upsell CTA.
  return <Explainer showCta={!partner} />
}
