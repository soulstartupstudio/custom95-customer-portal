...
export default function BrandshopUpsell({ compact = false }) {
  ...
  // Compact secondary card — used when Redeem is the primary focus on the page.
  if (compact) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-5">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex items-start gap-3 min-w-0">
            <div className="w-10 h-10 rounded-xl bg-gray-900 text-white flex items-center justify-center flex-shrink-0"><Store size={18} /></div>
            <div className="min-w-0">
              <div className="inline-flex items-center gap-1 text-[11px] font-semibold uppercase tracking-wide text-gray-500 mb-0.5"><Store size={11} />Brandshop</div>
              <h3 className="text-base font-semibold text-gray-900">Want a full branded store instead?</h3>
              <p className="text-sm text-gray-600 mt-0.5 leading-relaxed">
                A private white-label shop for your merch — you stock it (we warehouse) or run pre-orders, and we fulfil every order under your brand. More of a storefront than a redeem page.
              </p>
            </div>
          </div>
          <div className="flex-shrink-0">
            {state === 'sent' ? (
              <span className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-green-100 text-green-800 text-sm font-medium"><Check size={15} />Request sent</span>
            ) : (
              <button onClick={request} disabled={state === 'sending'} className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg border border-gray-300 text-gray-800 text-sm font-semibold hover:bg-gray-50 disabled:opacity-60">
                {state === 'sending' ? <Loader2 size={15} className="animate-spin" /> : <Store size={15} />}Ask about a Brandshop
              </button>
            )}
          </div>
        </div>
