...
import { BookOpen, Package, Leaf, Search, Globe } from 'lucide-react'
import { PageHeader, EmptyState, Spinner } from '../components/ui'
import CatalogueDetail from '../components/CatalogueDetail'

function CatalogueItemCard({ item, coloursByItem, onClick }) {
    ...
    <button onClick={onClick} className="bg-white rounded-xl border border-gray-200 overflow-hidden text-left hover:border-blue-300 hover:shadow-sm transition-all">
      <div className="aspect-square bg-gray-50 flex items-center justify-center overflow-hidden relative">
        ...
        <div className="absolute top-2 left-2 flex flex-wrap gap-1">
          {item.is_sustainable && <span className="bg-green-500/90 text-white text-[9px] font-medium px-1.5 py-0.5 rounded-full inline-flex items-center gap-0.5"><Leaf size={9} />EU</span>}
...
export default function CataloguePage({ company, contact, onStartProposalWithItem }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      // Fetch colour swatches for all visible items so cards can preview
      ...
      if (ids.length) {
        const { data: cs } = await supabase
          ...
          .eq('active', true)
        const byItem = {}
        for (const c of cs ?? []) (byItem[c.catalogue_item_id] = byItem[c.catalogue_item_id] || []).push(c)
        if (!cancelled) setColoursByItem(byItem)
      }
    ...
    <div className="space-y-6">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {filtered.map((i) => <CatalogueItemCard key={i.id} item={i} coloursByItem={coloursByItem} onClick={() => setSelected(i)} />)}
        </div>
      )}
